...
class purchasing extends CComponent {
    ...
    public function newPODelivery($hdr = array(), $dtl = array(), $rcpheader = array()) {
        ...
        if (!$deliveryhdr->save())
            return array('type' => 'E', 'message' => 'Error on create receipt', 'val' => $deliveryhdr->getErrors());
        else {
            $i = 0;
            foreach ($dtl['id_product'] as $value) {
                ...
                $deliverydtl->percent_disc = ($dtl['value_trans'][$i] > 0) ? ($dtl['value_disc'][$i] / $dtl['value_trans'][$i] * 100) : 0;

                if (!$deliverydtl->save())
                    return array('type' => 'E', 'message' => 'Error on Insert detail delivery', 'val' => $deliverydtl->getErrors());
                else if (count($rcpheader) > 0) {
